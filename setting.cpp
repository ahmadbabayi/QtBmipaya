    ...
    ui(new Ui::Setting)
{
    ...
    ui->KodePostiEdit->setValidator(new QRegularExpressionValidator(QRegularExpression("[0-9]*")));
    SettingReload();
}
...
void Setting::SettingReload(){
    ...
    while (query.next()) {
            ...
            ui->KodePostiEdit->setText(query.value(5).toString());
            ui->KodemelliEdit->setText(query.value(6).toString());
            ui->NameNomayandeEdit->setText(query.value(7).toString());
            ui->KodemelliNomayandeEdit->setText(query.value(8).toString());
            ui->BranchEdit->setText(query.value(9).toString());
            ui->BranchCodeEdit->setText(query.value(10).toString());
    }
}
...
void Setting::on_SaveButton_clicked()
{
    if (sehv()){
        QSqlQuery query;
        query.exec("UPDATE payasetting SET name = '"+ui->NameEdit->text()+"', sheba = '"+ui->ShebaEdit->text()+"', tel = '"+ui->TelEdit->text()+"', address = '"+ui->AddressEdit->text()+"', kodeposti = '"+ui->KodePostiEdit->text()+"', kodemelli = '"+ui->KodemelliEdit->text()+"', namenomayande = '"+ui->NameNomayandeEdit->text()+"', kodemellinomayande = '"+ui->KodemelliNomayandeEdit->text()+"', branchname = '"+ui->BranchEdit->text()+"', branchcode = '"+ui->BranchCodeEdit->text()+"' WHERE id =1");
