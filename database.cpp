...
void Database::dbcreate(){
    ...
    if (!QFile::exists("BmiPaya_data/bmipayayeni.db")){
        ...
        if (db.open()){
            ...
            query.exec("create table payasetting (id INTEGER PRIMARY KEY, name TEXT, sheba TEXT, tel TEXT, address TEXT, kodemelli TEXT, namenomayande TEXT, kodemellinomayande TEXT, branchname TEXT, branchcode TEXT)");
            query.exec("INSERT INTO payasetting (id, name, sheba, tel, address, kodemelli, namenomayande, kodemellinomayande, branchname, branchcode) VALUES (1,'','','','','','','','','')");
